document.addEventListener('DOMContentLoaded', () => {
    ...
    const play = (e, t) => e.play().catch(err => console.error(`${t} play failed:`, err));
    const pause = (e, t) => { e.pause(); if (t === "music") e.currentTime = 0; };
    ...
    const toggleMedia = () => {
        ...
        isVidBg ? (el.bgVideo.currentTime = 0, play(el.bgVideo, "Video"), play(el.bgMusic, "Music")) : (pause(el.bgVideo, "Video"), pause(el.bgMusic, "Music"));
    };
    ...
    const showNotif = (msg, confCB) => {
        ...
        notif.onclick = hide;
        document.getElementById("notification-box").onclick = ev => ev.stopPropagation();
    };
    ...
    const disableCtxMenu = () => document.addEventListener('contextmenu', e => e.preventDefault(), false);
    const disableTxtSel = () => { el.body.classList.add('no-select'); document.addEventListener('selectstart', e => e.preventDefault()); };

    const handlePageLink = e => { 
        e.preventDefault();
        showNotif("Warning: You are leaving coffeemanhell.", () => window.open(e.currentTarget.href, "_blank"));
    };
    ...
    el.body.addEventListener("click", ev => {
        ...
        if (btn) { ev.preventDefault(); showPage(btn.dataset.page); handleBtnClick(); }
    });
    ...
    window.addEventListener("load", () => {
        ...
        setTimeout(() => { el.loadBar.style.display = "none"; el.body.classList.remove("loading"); el.body.classList.add("loaded"); }, 1000); }, 1000);
        lazyLoad();
    });